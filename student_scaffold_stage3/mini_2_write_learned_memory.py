...
class MiniStageAgent(StudentAgent):
    ...
    def run_session(self, session, runtime: StudentRuntime):  # noqa: ANN001
        """Mini-stage 2: add your learned memory writer to the Stage-2 loop."""

        # -- YOUR TASK ---------------------------------------------------------
        # STEP A: Call self.extractor.predict(session.user_message).
        #         If the label is morning or afternoon, write to memory.
        #         If the label is none, skip the write.
        #
        # STEP B: Same as always — search memory, set active_preference.
        #
        # The loop below is the Stage-2 loop unchanged. You do not need to
